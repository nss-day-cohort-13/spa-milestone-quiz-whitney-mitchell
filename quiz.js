...
function populatePage (inventory) {
  ...
  for (var i=0; i < inventory.length; i++) {
  	if (i%3 === 0) {
  		carString += "<div class = 'row-fluid'>";
  	}
    // Below, border styling in script: ONLY because theuser shouldn't
    // have to edit the CSS to add a new car into inventory.
    // Works, but I'll research a better way after all requirments are met.
  	carString += "<div class = 'col-sm-4 card' id = 'card" + i + "' style='border: 3px solid " + inventory[i].color + "'><p class='mainInfo'>" + inventory[i].year + " " + inventory[i].make + " " + inventory[i].model + "</p>";
